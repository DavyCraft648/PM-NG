...
class InGamePacketHandler extends ChunkRequestPacketHandler{
	...
	private function onFailedBlockAction(Vector3 $blockPos, ?int $face) : void{
		if($blockPos->distanceSquared($this->player->getLocation()) < 10000){
			...
			foreach($this->player->getWorld()->createBlockUpdatePackets($this->session->getTypeConverter()->getBlockTranslator(), $blocks) as $packet){
			...
			if($textBlobTag instanceof StringTag){
				...
				try{
					if(!$block->updateText($this->player, $text)){
						foreach($this->player->getWorld()->createBlockUpdatePackets($this->session->getTypeConverter()->getBlockTranslator(), [$pos]) as $updatePacket){
